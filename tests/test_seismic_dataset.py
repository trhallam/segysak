...
from segysak._seismic_dataset import _check_vert_units, _check_input, _dataset_coordinate_helper
from segysak._seismic_dataset import create_seismic_dataset, create2d_dataset, create3d_dataset
...
class TestCheckers:
    ...
    @pytest.mark.parametrize("u", VerticalUnits._member_names_)
    ...
    def test_illegal_vertical_units_raise_errors(self, t):
        assume(t not in VerticalUnits._member_names_)
        with pytest.raises(ValueError):
            _check_vert_units(t)

    @pytest.mark.parametrize("p", [d.value for d in VerticalKeyField])
    ...
    def test_illegal_domain_raise_errors(self, t):
        assume(t not in [d.value for d in VerticalKeyField])
        with pytest.raises(ValueError):
...
class TestCreateSeismicDataset:
    ...
    def test_create_2D_seismic_dataset_with_integers(self, s, t):
        dataset = create_seismic_dataset(twt=s, depth=None, cdp=t, iline=None, xline=None, offset=None)
    ...
    def test_create_2D_seismic_dataset_with_offsets(self, o):
        dataset = create_seismic_dataset(twt=100, depth=None, cdp=1000, iline=None, xline=None, offset=o)
    ...
    def test_create_3D_seismic_dataset_with_integers(self, s, i, x):
        dataset = create_seismic_dataset(twt=None, depth=s, cdp=None, iline=i, xline=x, offset=None)
        assert len(dataset.dims) == 3
        assert dataset.dims['depth'] == s

    @given(arrays(np.float, shape=integers(0, 10000), elements=floats(-1000, 1000)), integers(0, 10000))
    def test_create_2D_seismic_dataset_with_arrays(self, a, t):
        dataset = create_seismic_dataset(twt=a, depth=None, cdp=t, iline=None, xline=None, offset=None)
        assert len(dataset.dims) == 2
    ...
    def test_create_2D_seismic_dataset_with_multiple_dimensions(self, d):
        ...
        dataset = create_seismic_dataset(twt=100, depth=None, cdp=1000, iline=None, xline=None, offset=None, **dims)
...
class TestCreate2DDataset:
    ...
    def test_create_2D_dataset_custom_sampling(self, s, f, r):
        dataset = create2d_dataset(dims=(100, s), first_sample=f, sample_rate=r, vert_domain="TWT")
...
class TestCreate3DDataset:
    ...
    """
    @given(tuples(integers(1, 10000), integers(1, 10000), integers(0, 1000)), sampled_from(VerticalUnits._member_names_))
    ...
    @given(integers(15, 60), floats(0, 15), floats(1, 15), sampled_from(["TWT", "twt", "DEPTH", "depth"]))
    def test_create_angle_stack_dataset(self, o, f, s, d):
        dataset = create3d_dataset((1000, 1000, 100, o), first_offset=f, offset_step=s, vert_domain=d)
