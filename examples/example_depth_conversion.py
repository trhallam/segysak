...
volve_ds = volve_ds.rename({'samples':'twt',})
...
#
# The velocity cube is in depth. So we will need to create a mapping to TWT to match the input seismic data. The mapping is essential a TWT cube sampled in depth. When depth conversion is performed we interpolate or sample the seismic data to these TWT time values which implicitly gives the new seismic volume in depth. Lets think about why we have to do it this way.
#
# If we just calculated the depth for each TWT sample in our original seismic cube using the provided velocities, we would end up with irregular sampling in the depth domain. Furthermore, the depth axis samples would be different for each trace, which have unique velocity functions. We usually need our seismic data on a regular grid, so this is no good.
#
# The velocities are migration interval velocities and represent the velocity in the interval of the sample. Lets look at a single trace with smooth plotting (left) and step based plotting (right).
...
# %% [markdown]
# To calculate the two-way-time (TWT) of each velocity sample to create a mapping between depth and time the velocity must be integrated. The easiest way to do this, is to divide the velocity interval (depth step) by the associated velocity and then sum the results. The Xarray function to perform the sum iteratively along an axis is the cumulative sum or `cumsum`. The integration is also scaled from seconds to milliseconds to match the data vertical axis and doubled (for two way travel time).
#
# Using the Xarray `cumsum` here is important. As our data is lazily loaded, Xarray/Dask have been building a computation graph, this describes the steps we want to take, but doesn't actually execute any calculations until we need them. This allows us to minimise the amount of computation needed. To display these trace functions, only a small amount of data is loaded and calculations carried out. Thats why its quite fast. Ideally, we should use Xarray methods, as they handle the application of functions and the computational graph. If we were to apply NumPys `cumsum` instead, we would break this flow and force a full computation to occur. The lesson here, is stay within the Xarray/Dask API if you can.
#
...
# %% [markdown]
# We will call this new twt to depth mapping the variable `twtc` and assign it back to the velocity data volume. 
# Remember, `twtc` links the depth dimension to a TWT which we will need to convert the seismic from twt to depth.
#
# This variable `twtc` again exists in the computation graph, and no calculations have yet been carried out. In fact, our data isn't even
# loaded into memory.
...
#
# The velocity volume must now be upsampled (interpolated) from the sparse velocity cube to match the seismic data. This will create a trace in the velocity volume for each trace in the seismic volume. The method of interpolation can be simple like `linear` but we will use `cubic` or spline based interpolation. Xarray interpolation uses the `scipy` library.
...
# %% [markdown]
# Here we show the original (left) and upsampled data (right), velocity (top) and TWT (bottom). The vertical axis is depth.
...
# %%
twtc_subsampled_3m = vel_dsi['twtc'].interp(coords={'depth':np.arange(0, 4500, 3)}, method='cubic')
...
def trace_resample(trace, var_name, seismic_var='data', out_var='twtc', method='cubic'):
    ...
    trace_z = trace[seismic_var].interp(method='cubic', coords={in_dim:trace[out_var].squeeze().values})
...
# %% editable=true slideshow={"slide_type": ""}
volve_dsZ = volve_ds.map_blocks(trace_resample, args=('dataZ',), template=volve_ds['twtc'])
volve_dsZ.name = 'data'
...
twt_plot = volve_ds.sel(iline=10095).data.T.plot(yincrease=False, ax=axs[0], cmap='Greys')
...
zv_plot = vel_dsi.sel(iline=10095).migintvel.T.plot(yincrease=False, ax=axs[1], alpha=.3)
...
# %%
ds = xr.open_dataset('data/outZ.segy', dim_byte_fields={'iline':189, 'xline':193}, 
    extra_byte_fields={'cdpx':181, 'cdpy':185},)
